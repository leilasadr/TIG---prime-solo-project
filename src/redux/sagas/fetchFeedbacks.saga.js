...
function* fetchFeedbacks() {
    try{
        const feedbacks = yield axios.get('/api/feedback');
        console.log('Here are our feedbacks:', feedbacks.data);
        yield put({
            type: 'SET_FEEDBACKS', payload: feedbacks.data
        })
    } catch{
        console.log('Error connecting to server, could not GET our feedbacks');
    }
}
